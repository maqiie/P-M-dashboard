...
export const AuthProvider = ({ children }) => {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      const finalUser = userData.data || userData || storedUser;
      console.log('Check auth - user data:', finalUser);
  ...
  const verifyOtp = async (otp) => {
    try {
      ...
      console.log("User object from OTP:", response.user);
  
      if (response.accessToken) {
        // FIXED: Get role from the user object
        ...
        // CRITICAL: Set the user with the role
        const userWithRole = {
          ...response.user,
          role: role
        };
        
        console.log("Setting user in context:", userWithRole);
        setUser(userWithRole);
        setIsAuthenticated(true);
  
        return { ...response, user: userWithRole, role: role };
      } else {
  ...
  // FIXED: Helper functions for role checking
  const isAdmin = () => {
  ...
  const hasRole = (role) => {
    console.log('hasRole check - requested:', role, 'user.role:', user?.role);
