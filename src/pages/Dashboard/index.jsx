...
// Example usage within the UserDashboard component
const UserDashboard = () => {
  const [projects, setProjects] = useState([]);

  const handleCreateProject = (projectData) => {
    setProjects([...projects, projectData]);
    // Here you would typically make an API call to save the project data
  };
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 lg:gap-4 mb-6 lg:mb-8">
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-xl lg:rounded-2xl p-3 lg:p-4 border border-white/30 shadow-lg hover:shadow-xl transition-shadow">
                <div className="flex items-center space-x-2 lg:space-x-3">
                  ...
                  <div>
                    <p className="text-lg lg:text-2xl font-bold text-gray-900">
                      {calculatedStats.totalTeamMembers}
                    </p>
                    <p className="text-xs text-gray-600">Team Members</p>
                  </div>
                </div>
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-xl lg:rounded-2xl p-3 lg:p-4 border border-white/30 shadow-lg hover:shadow-xl transition-shadow">
                <div className="flex items-center space-x-2 lg:space-x-3">
                  ...
                  <div>
                    <p className="text-lg lg:text-2xl font-bold text-gray-900">
                      {calculatedStats.upcomingEvents}
                    </p>
                    <p className="text-xs text-gray-600">This Week</p>
                  </div>
                </div>
                    ...
                    {myProjects.length > 0 ? (
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
                        {myProjects.map((project) => (
                          <div
                            key={project.id}
                            className="bg-gradient-to-br from-white to-gray-50/50 rounded-lg lg:rounded-xl p-4 lg:p-6 border border-gray-100 hover:shadow-lg transition-all duration-300 group relative overflow-hidden"
                          >
                            {/* Priority Indicator */}
                            <div
                            ...
                            <div className="flex items-start justify-between mb-3 lg:mb-4">
                              <div className="flex-1">
                                ...
                                <span
                                  className={`inline-flex px-2 lg:px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(
                                    project.status
                                  )}`}
                                >
                                  {project.status.replace("_", " ")}
                    ...
                    {recentTasks.length > 0 ? (
                      <div className="space-y-2 lg:space-y-3">
                        {recentTasks.slice(0, 4).map((task) => (
                          <div
                            key={task.id}
                            className={`p-2 lg:p-3 rounded-lg border-l-4 ${
                    ...
                    {upcomingEvents.length > 0 ? (
                      <div className="space-y-2 lg:space-y-3">
                        {upcomingEvents.slice(0, 3).map((event) => (
                          <div
                            key={event.id}
                            className="flex items-center space-x-2 lg:space-x-3 p-2 lg:p-3 hover:bg-gray-50 rounded-lg transition-colors"
                          >
                            <div
                              className={`w-1 lg:w-2 h-8 lg:h-10 rounded-full ${
                                event.type === "meeting"
                                  ? "bg-blue-400"
                                  : event.type === "review"
                                  ? "bg-purple-400"
                    ...
                    {tenders.length > 0 ? (
                      <div className="space-y-2 lg:space-y-3">
                        {tenders.map((tender) => (
                          <div
                            key={tender.id}
                            className="p-2 lg:p-3 bg-gradient-to-r from-orange-50 to-red-50 rounded-lg border border-orange-200"
                          >
                            <h4 className="font-medium text-gray-900 text-xs lg:text-sm mb-1 lg:mb-2">
                              {tender.title}
                            </h4>
                            <div className="flex items-center justify-between text-xs">
